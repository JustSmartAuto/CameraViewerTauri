 ...
 * ----------------------------------------------------------------
 * 本脚本通过 Tauri WebviewWindowBuilder.initialization_script 在“文档创建期”
 ...
 *  - 语言完全跟随软件：默认英文（原文），父页面通过 postMessage 通知语言；
...
(function () {
  ...
  function notifyReady() {
    try { window.parent.postMessage({ __hmiI18n: "ready" }, "*"); } catch (e) {}
  }

  // ==================== 与父页面的语言同步 ====================
  window.addEventListener("message", function (e) {
    // 只接受来自直接父窗口的消息（消息体无敏感操作，仅切换语言）
  ...
  // 初始即上报一次（父窗口可能尚未绑定监听，父窗口在 load 时也会主动下发）
  notifyReady();
})();
